...
**Note:** This vignette demonstrates advanced cell-type harmonization workflows
that require external dependencies (Python with OnClass, scKirby package).
Code is shown but not executed. See the tf-idf vignette for a working example.

```{r setup, include=FALSE}
## NOTE: eval=FALSE globally because this vignette requires:
## - Python with OnClass installed
## - scKirby package (not currently a dependency)
## - Local file paths that won't exist on CI
knitr::opts_chunk$set(echo = TRUE, eval = FALSE, fig.width = 7, fig.height = 5)
```
...
mat <- data.table::data.table(relate$data) %>%
  ...
  as.matrix() 
rowmax <- DelayedArray::rowMaxs(mat, na.rm = TRUE)
