...
#'  as the number of variable genes overall (\code{TRUE}) or per subset
#'  (\code{FALSE}).
#' @param return_nested Return a named list of variable genes per subset
#'  (\code{TRUE}) or a flattened list (\code{FALSE}).
#' @param nfeatures_max The max number of features to return.
#' @param batches Number of batches.
#' Features are prioritised according to how many variable gene subsets
#' they appear in.
#'
...
#'     nfeatures_max = 50
#' )
FindVariableFeatures_split <- function(
        seurat,
        split.by,
        nfeatures = 4000,
        count_overall = FALSE,
    ...
    if (return_nested) {
        var_features <- lapply(var_features, function(x) {
            x[x %in% top_features]
        }) |> `names<-`(splits)
        return(var_features)
    } else {
