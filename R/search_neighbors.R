...
#' @param label_col \code{meta.data} column used to name the rows/columns of
#' the graph.
#' \code{label_col} will also be used in the search for \code{var1_search}
#' substring.
...
search_neighbors <- function(seurat,
    ...
    if (length(var1_search) > 1) {
        var1_search <- paste(var1_search, collapse = "|")
    }
    ...
    if (!is.null(group_col)) {
        messager("+ Filtering results by `var2_group`:",
                 var2_group, v = verbose)
        targets2 <- seurat@meta.data[
            grepl(var2_group, seurat@meta.data[[group_col]], ignore.case = TRUE),
            "sample_names"
        ]
